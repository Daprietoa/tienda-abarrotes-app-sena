...
class ProductService:
    ...
    async def change_product(self, product_data: Product, product_id: int):
        try:
            self.con.ping(reconnect=True)
            with self.con.cursor() as cursor:
                sql = """
    ...
    async def search_products_by_name(self, nombre: str):
        """Busca productos por nombre con LIKE"""
        try:
            self.con.ping(reconnect=True)
            with self.con.cursor(pymysql.cursors.DictCursor) as cursor:
                sql = "SELECT * FROM producto WHERE nombre_producto LIKE %s"
                like_param = f"%{nombre}%"
